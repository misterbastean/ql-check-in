...
router.post('/students/new', (req, res) => {
  ...
  const newStudent = utils.newStudent(req);
...
router.get('/students/:id', (req, res) => {
  ...
  Student.findOne({sid: req.params.id}, (err, student) => {
    if(err) {
      ...
      const missingPunches = utils.punchLoop(student, timeBlocks);

      // Remove invalid time blocks (missing punch in or out)
      timeBlocks.forEach((block) => {
        if (block.in == 'Student did not punch in' || block.out == 'Student did not punch out') {
