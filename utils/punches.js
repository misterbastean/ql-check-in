module.exports = {
  punchLoop: function (student, timeBlocks) {
    ...
    while (i < student.punches.length) {
      if (student.punches[i].type === 'in' && student.punches[i+1].type === 'out') {
        timeBlocks.push({
          in: student.punches[i].rawDate,
          out: student.punches[i+1].rawDate
        });
        i += 2;
      } else if (student.punches[i].type === 'in' && student.punches[i+1].type === 'in') {
        ...
        i++;
      } else if (student.punches[i].type === 'out' && student.punches[i+1].type === 'in') {
        timeBlocks.push({
